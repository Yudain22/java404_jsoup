...
//        System.out.println("다음 홈페이지");
//        try {
//            Document google1 = Jsoup.connect("https://www.daum.net").get();
//            System.out.println(google1);
//        } catch (IOException e) {
//            throw new RuntimeException(e);
//        }
//        System.out.println("구글 홈페이지");
//        try {
//            Connection.Response res = Jsoup.connect("https://www.google.co.kr")
...
//        }
        ...
        //select()를 사용하여 지정한 태그를 제외한 나무저ㅣ 태그를 모두 가져오기
        Elements elements = doc.select(".o3j99");
        System.out.println("class 값 .03j99 로 검색 결과 :" +elements.size());
        ...
        //select()를 사용하여 지정한 태그를 제외한 나머지 태그를 모두 가젿오기
        //전체 html 문서에서 클래스 값이 L3eUfb인 태그 중 가장 먼저 검색되는 태그를 한 개 가져옴
        Element element = doc.select(".L3eUgb").first();

        //클래스 값이 L3eUfb 인 태그 안에서 태그의 자식 태그 중 div 태그를 모두 가져옴
        divList1 = element.select("div");
        //클래스 값이 L3eUfb 인 태그 안에서 태그의 자식 태그 중 div 태그를 모두 가져오는데 클래스 값이 03j99인 태그는 제외함
        divList2 = element.select("div").not(".o3j99");

        System.out.println(".03j99 포함 : "+ divList1.size()+"개");
        System.out.println(".03j99 미포함 : " + divList2.size()+"개");
...
//        System.out.println(divTag);
        ...
        System.out.println("img 의 src 값 가져오기 : " + attrValue);
