...
public class DaumNews {
    ...
    public static void main(String[] args) {
        ...
        //오늘의 이슈 리스트 목록 가져오기
        //클래스 속성 값이 list_newsissue인 태그 선택
        Element list_newsissue = html.select(".list_newsissue").first();

        //ul 태그의 자식인 태그인 li 태그를 모두 가져옴
//        Elements li_list = list_newsissue.select("li");
//        System.out.println(li_list.size());

        //ul 태그의 자식 태그인 li 태그의 자식 태그가 class 값이 item_issue 인 div 태그 하나 밖에 없으므로 ul 태그에서 div 태그를 바로 검색해서 모두 가져옴
...
//        System.out.println(title);
        ...
        for (int i =0; i<item_issue.size(); i++) {
            //class 속성값이 item_issue인 태그를 get()를 통해서 하나씩 가져옴
            Element item = item_issue.get(i);
            //item에서 class 속성값이 cont_thumb인 태거를 검색하여 출력
            //first()를 사용하여 가장 먼저 검색된 1개만 가져옴
            //class가 item_issue인 태그 안에서 class 가 cont_thumb인 태그는 1개 밖에 없음

//            System.out.println(item.select(".link_txt").size());
//            Element cont_thumb = item.select(".cont_thumb").first();
//            //class 가 cont_thumb인 태그 안에서 strong 태그 검색, cont_thumb 안에는 strong 태그가 1개 밖에 없음, first()를 사용하여 1개만 가져옴
//            Element strong = cont_thumb.select("strong").first();
